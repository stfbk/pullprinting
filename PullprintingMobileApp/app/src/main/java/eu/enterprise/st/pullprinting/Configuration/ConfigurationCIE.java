package eu.enterprise.st.pullprinting.Configuration;
...
import eu.enterprise.st.pullprinting.Utilities.ConnectionBuilderForTesting;
import okio.Buffer;
import okio.BufferedSource;
import okio.Okio;

public final class ConfigurationCIE {

    private static final String TAG = "ConfigurationCIE";

    private static final String PREFS_NAME = "configCIE";
    private static final String KEY_LAST_HASH = "lastHashCIE";

    private static WeakReference<ConfigurationCIE> sInstance = new WeakReference<>(null);
    ...
    private Uri mcloudlist;
    private boolean mHttpsRequired;

    public static ConfigurationCIE getInstance(Context context) {
        ConfigurationCIE config = sInstance.get();
        if (config == null) {
            config = new ConfigurationCIE(context);
            sInstance = new WeakReference<ConfigurationCIE>(config);
        }

        return config;
    }

    public ConfigurationCIE(Context context) {
        mContext = context;
    ...
    private void readConfiguration() throws InvalidConfigurationException {
        BufferedSource configSource =
                Okio.buffer(Okio.source(mResources.openRawResource(R.raw.auth_config_sc)));
        ...
        if (getConfigString("discovery_uri") == null) {
            ...
            mcloudlist = getRequiredConfigWebUri("job_endpoint_uri");
